                ...
                <table class="table table-report -mt-2">
                    ...
                    <tbody>
                        <?php foreach ($invoice as $row): ?>
                            <tr class="intro-x">
                                <td class="w-40 !py-4"> <a href="<?= site_url(
                                    'admin/invoice/detail/' . $row->order_id
                                ...
                                <td><?php if (empty($row->gambar)) { ?>
                                   ...
                                   <?php } else { ?>
                                    <a href="">
                                        <div class="flex items-center whitespace-nowrap text-primary"> <i data-lucide="link" class="w-4 h-4 mr-2"></i><a href="<?= base_url() .
                                            '/uploads/' .
                                            $row->gambar ?>">Lihat Bukti </a></div>
                                    </a>
                                    <?php } ?>
                                </td>
                                <td>
                            ...
                            <td class="table-report__action"><center>
                                <?php if ($row->status == '0') { ?>
                                    <div class="flex justify-center items-center">
                                        <a class="flex items-center text-primary whitespace-nowrap" href="<?= site_url(
                                            'admin/invoice/confirm/' .
                                                $row->order_id
                                        ) ?>"> <i data-lucide="arrow-left-right" class="w-4 h-4 mr-1"></i> Ubah Status </a>
                                    </div>
                                <?php } elseif ($row->status == '1') { ?>
                                    <button class="btn btn-sm btn-success text-white">Pembayaran Sukses</button>
                                <?php } ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
